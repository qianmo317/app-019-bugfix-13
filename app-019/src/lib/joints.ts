...
export function computeDowel(input: DowelInput): DowelResult {
  ...
  const positions = Array.from({ length: count }, (_, k) => round01(step * k))
  const edgeMargin = round01(step)
  if (thickness < 12) warnings.push('板厚不足 12mm，圆木榫易穿透板面，建议改用饼干榫或拼板槽')
  void input.kerf
  return { dowelDia, dowelLength: dowelDia * 5, holeDepth: round01((dowelDia * 5) / 2 + 1), count, positions, edgeMargin, warnings }
}
