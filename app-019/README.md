...
板宽等分 N 段，孔打在各段中点：**两端各留半个孔距**，首末孔均匀落在板内、不压板边；两件从同一基准端起量，孔位一一对应。0.1mm 网格累积取整，端距/孔距标注取实际孔位（如 200mm 板 3 孔 → 33.4 / 100 / 166.7，端距 ≈33.3、孔距 66.7）。端距不足 2×榫径或饼干榫半长时给劈裂警告。
...
| 层级 | 数量 | 覆盖点 |
| --- | --- | --- |
| 单元（vitest） | 53 | 随机 200 组燕尾闭合 ≤ 0.1mm（mulberry32 固定种子）、直榫 10 组手工核算、圆榫/饼干榫布孔 10 例（端距=孔距/2、首末孔不压边、闭合）、三视图一致性、边界警告、锯切线数量、导出导入往返全等、性能 |
